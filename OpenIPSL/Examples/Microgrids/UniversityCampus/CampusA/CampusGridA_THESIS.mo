...
model CampusGridA_THESIS "Microgrid model for university campus A"
  ...
  PfData.PowerFlow pf(redeclare record PowerFlow =
        OpenIPSL.Examples.Microgrids.UniversityCampus.CampusA.PfData.Pf00001)
  ...
  Electrical.Events.Breaker BreakerMicrogrid(
    ...
    t_o=2,
    rc_enabled=true,
    t_rc=50)  annotation (Placement(transformation(
        extent={{-6,-6},{6,6}},
        rotation=270,
        origin={0,180})));
...
equation
  ...
  connect(BreakerMicrogrid.s, AENA1.p)
    annotation (Line(points={{0,186},{0,194}}, color={0,0,255}));
  connect(BreakerMicrogrid.r, AENA.p)
